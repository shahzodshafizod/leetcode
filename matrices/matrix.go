...
func (m *matrix) BFS() []int {
	...
	var values = make([]int, 0, m.height*m.width)
	for !queue.Empty() {
		var current = queue.Dequeue()
		var row, col = current[0], current[1]

		values = append(values, m.grid[row][col])

		for dir := 1; dir < 5; dir++ {
			var r = row + m.dirs[dir-1]
			var c = col + m.dirs[dir]
