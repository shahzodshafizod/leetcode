...
func TestTricks(t *testing.T) {
	...
	assert.Equal(
		...
		(0b_0101_1000>>2)&1,
		0,
	) // starting from 0 from rightmost, the second bit isn't set
	assert.Equal(t, (0b_0101_1000>>3)&1, 1) // starting from 0 from rightmost, the third bit is set

	// 2. check if a number is even
	assert.Equal(t, 46&1, 0)
	assert.NotEqual(t, 47&1, 0)
	...
	for num, is := range map[int]bool{32: true, 47: false, 0: false} {
		...
		assert.Equal(t, num != 0 && num&(num-1) == 0, is)
	}
}
...
func TestBits(t *testing.T) {
	...
	// 0b01010111 =
	// 0b01010000
	assert.Equal(t, 0b01011000&0b01010111, 0b01010000)

	// OR
	// 0b01011000 |
	// 0b01010111 =
	// 0b01011111
	assert.Equal(t, 0b01011000|0b01010111, 0b01011111)

	// XOR (exclusive OR)
	// 0b01011000 ^
	// 0b01010111 =
	// 0b00001111
	assert.Equal(t, 0b01011000^0b01010111, 0b00001111)
