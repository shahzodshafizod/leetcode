...
class Solution(unittest.TestCase):
    ...
    def test(self):
        for mountainArr, target, expected in [
            ...
            # ([11,21,29,38,39,40,47,49,50,54,57,60,61,69,78,81,83,92,102,104,109,114,123,132,135,143,144,153,163,166,167,175,173,165,155,145,136,126,118,110,104,103,100,92,87,77,71,65,62,59,57,52,49,41,40,34,30,28,24,15,7,0], 92, 17),
        ]:
            mountainArr = MountainArray(mountainArr)
            output = self.findInMountainArray(target, mountainArr)
            output = self.assertEqual(expected, output, f"expected: {expected}, output: {output}")
