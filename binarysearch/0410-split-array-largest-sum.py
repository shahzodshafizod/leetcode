...
# python3 -m unittest binarysearch/0410-split-array-largest-sum.py
...
class Solution(unittest.TestCase):
    ...
    #             minmaxsum = min(minmaxsum, max_sum)
    #             if curr_sum > minmaxsum: # optimization
    #                 break
    #         dp[(idx,k)] = minmaxsum
    #         return minmaxsum
    #     return dfs(0, k)
    ...
    def splitArray(self, nums: List[int], k: int) -> int:
        ...
        while left <= right:
            ...
            subarrays = 1  # we're in the first subarray now
            curr_sum = 0
            for num in nums:
                curr_sum += num
                if curr_sum > mid:
