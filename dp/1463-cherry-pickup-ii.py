        ...
        for row in range(m - 1, -1, -1):
            ...
            for col1 in range(n):
                for col2 in range(col1 + 1, n):
                    for nc1 in range(max(0, col1 - 1), min(n - 1, col1 + 1) + 1):
                        for nc2 in range(max(0, nc1 + 1, col2 - 1), min(n - 1, col2 + 1) + 1):
                            dp[curr][col1][col2] = max(dp[curr][col1][col2], dp[nxt][nc1][nc2])
        ...
        for grid, expected in [
            ...
            (
                [
                    ...
                    [0, 52, 0, 0, 0, 0, 0, 0, 48, 0],
                    [0, 0, 94, 0, 0, 0, 0, 74, 0, 0],
                    [0, 0, 0, 98, 0, 0, 25, 0, 0, 0],
                    [0, 0, 0, 0, 70, 66, 0, 0, 0, 0],
                ],
                617,
            ),
            # ([[0,40,0,0,0,0,0,0,93,0],[0,0,41,0,0,0,0,59,0,0],[89,0,0,0,0,0,0,0,0,28],[0,32,0,0,0,0,0,0,80,0],[0,0,40,0,0,0,0,0,0,0],[50,0,0,0,0,0,0,0,0,66],[0,90,0,0,0,0,0,0,11,0],[0,0,62,0,0,0,0,12,0,0],[95,0,0,0,0,0,0,0,0,88],[0,31,0,0,0,0,0,0,26,0]], 686), # pylint: disable=line-too-long
