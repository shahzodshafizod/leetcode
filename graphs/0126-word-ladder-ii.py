...
class Solution(unittest.TestCase):
    ...
    def findLadders(self, beginWord: str, endWord: str, wordList: List[str]) -> List[List[str]]:
        ...
        exit = False
        layer = 0
        while queue and not exit:
            layer += 1
            for _ in range(len(queue)):
                word = queue.popleft()
                for idx in range(m):
                    pattern = word[:idx] + '*' + word[idx+1:]
                    for next in graph[pattern]:
                        if next not in seen or seen[next] == layer+1:
                            exit = exit or next == endWord
    ...
    def testFindLadders(self) -> None:
        for beginWord, endWord, wordList, expected in [
            ...
            ("ta", "if", ["ts","sc","ph","ca","jr","hf","to","if","ha","is","io","cf","ta"], [["ta","ca","cf","if"],["ta","ha","hf","if"],["ta","ts","is","if"],["ta","to","io","if"]]),
        ]:
