...
class Solution(unittest.TestCase):
    ...
    def closestMeetingNode(self, edges: List[int], node1: int, node2: int) -> int:
        ...
        while n1.node != -1 or n2.node != -1:
            ...
            if seen[n1.node+1] & n1.mask: n1.node = -1
            if seen[n2.node+1] & n2.mask: n2.node = -1

            seen[n1.node+1] |= n1.mask
            seen[n2.node+1] |= n2.mask

            if seen[n1.node+1] == 3: return n1.node
            if seen[n2.node+1] == 3: return n2.node

            if n1.node != -1: n1.node = edges[n1.node]
            if n2.node != -1: n2.node = edges[n2.node]
        return -1
